...
//4 bits move flag
{
    public partial class CSearch
    {
        ...
        int Search(MList ml, NodeType nt, int ply, int depth, int alpha, int beta)
        {
            int oldAlpha = alpha;
            int bestMove = 0;
            int bestValue = alpha;
            ...
            if (!usChecked && alpha == beta - 1)
            {
                ...
                if (optNullPruning && (ply > 1) && !usChecked && (depth > 2) && CPosition.NotOnlyPawns())
                {
                    hash ^= hashColor;
                    CPosition.ChangeColor();
                    int score = -Search(ml, NodeType.NonPv, ply + 1, depth - 4 - depth / 6, -beta, 1 - beta);
                    hash ^= hashColor;
                    CPosition.ChangeColor();
                    if (score >= beta)
                        return beta;
            ...
            int legalMoves = 0;
            CMovePicker movePicker = new CMovePicker(ml, hash, killers[ply, 0], killers[ply, 1]);
            while (movePicker.NextMove(out Move move))
            {
                ...
                if (CPosition.IsLegal())
                {
                    ...
                    {
                        MList ml2 = CMovesGenerator.GenerateMovesAll();
                        score = alpha + 1;
                        if (legalMoves > 0)
                        {
                            if (reducible && !CPosition.IsCheck())
                                score = -Search(ml2, NodeType.NonPv, ply + 1, depth - 2, -alpha - 1, -alpha);
                            if (score > alpha)
                                score = -Search(ml2, NodeType.NonPv, ply + 1, depth - 1, -alpha - 1, -alpha);
                        }
                        if ((legalMoves == 0) || ((score > alpha) && (score < beta)))
                            score = -Search(ml2, NodeType.Pv, ply + 1, depth - 1, -beta, -alpha);
                    }
                    legalMoves++;
                }
                UnmakeMove(move);
                ...
                if (score > alpha)
                {
                    ...
                    if (Program.dataIn.post && (ply == 0))
                    {
                        ...
                        Console.WriteLine($"info depth {depthCur} score {scFm} multipv {curPv} time {Convert.ToInt64(t)} nodes {nodeCur} nps {Convert.ToInt64(nps)}  seldepth {bstMoves.Count} currmove {bsFm} currmovenumber {legalMoves} hashfull {CTranspositionTable.Permill()} pv {pv}");
                    }
                }
            }
            if (!engineStop)
        ...
        public void Start(MList mo, int depth, int time, ulong nodes)
        {
            ...
            do
            {
                int score = Search(mc, NodeType.Root, 0, depthCur, -Constants.CHECKMATE_MAX, Constants.CHECKMATE_MAX);
                ...
                curPv++;
                if ((curPv > optMultiPv) || (mc.count==0))
                {
                    curPv = 1;
                    mo.CopyTo(mc);
                    depthCur++;
        ...
        public void UciGo()
        {
            ...
            if (searchmoves.count > 0)
                searchmoves.CopyTo(mo);
            else
                mo = GenerateMovesLegal();
            if (mo.count == 0)
            {
