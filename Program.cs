    ...
    class Program
    {
        ...
        string version = "2019-04-03";
                ...
                while (n-- > 0)
                {
                    ...
                    if (alpha < osScore)
                    {
                        ...
                        if (depth == 1)
                        {
                            ...
                            Console.WriteLine("info currmove " + bsFm + " currmovenumber " + n + " nodes " + g_totalNodes + " time " + t + " nps " + nps + " depth " + g_depthout + " seldepth " + alphaDe + " score " + g_scoreFm + " pv " + bsPv);
                        }
                    }
                    if (alpha >= beta) break;
                }
            ...
            void Search(int depth,int time,int nodes)
            {
                ...
                } while (((depth == 0) || (g_depthout < depth)) && (g_depth <= bsDepth) && !g_stop && (m1 > 0));
                double t = (DateTime.Now - g_startTime).TotalMilliseconds;
                int nps = 0;
                if(t > 0)
                    nps = Convert.ToInt32((g_totalNodes / t) * 1000);
